...
/*   Updated: 2017/12/20 20:08:06 by lportay          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	user_input(t_21sh *env, char *buf)
{
	...
	{
		while (funcs[i].test && funcs[i].test(env, buf) == false)
			i++;
		if (funcs[i].test)
			funcs[i].f(env);
	}
...
void		lineread(t_21sh *env)
{
	...
	while (status == READON)
	{
		ft_bzero(buf, READMAXLEN);
		if (read(STDIN_FILENO, buf, READMAXLEN) == -1)
			status = READERROR;
		else
			status = user_input(env, buf);
	}
