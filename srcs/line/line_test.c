...
/*   Updated: 2017/12/20 19:00:52 by lportay          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!ft_strncmp(buf, C_RIGHT, ft_strlen(C_RIGHT)) && env->line->next)
		return (true);
	if (!ft_strncmp(buf, M_F, ft_strlen(M_F)) && env->emacs_mode && env->line->next)
		return (true);
	else
		return (false);
}
...
{
	if (!ft_strncmp(buf, C_LEFT, ft_strlen(C_LEFT)) && env->line->previous)
		return (true);
	else if (!ft_strncmp(buf, M_B, ft_strlen(M_B)) && env->emacs_mode && env->line->previous)
		return (true);
	else
		return (false);
}
...
{
	if (!ft_strncmp(buf, C_UP, ft_strlen(C_UP)) && (env->cursor_offset >= env->ws.ws_col))
		return (true);
	else
		return (false);
}
...
{
	if (!ft_strncmp(buf, C_DOWN, ft_strlen(C_DOWN)) && (env->cursor_line < env->num_lines))
		return (true);
	else
		return (false);
}
...
{
	if (!ft_strncmp(buf, END, ft_strlen(END)) && env->line->next)
		return (true);
...
{
	if (!ft_strncmp(buf, HOME, ft_strlen(HOME)) && env->line->previous) 
		return (true);
...
{
	if (!ft_strncmp(buf, RIGHT_KEY, ft_strlen(RIGHT_KEY)) && env->line->next)
		return (true);
...
{
	if (!ft_strncmp(buf, LEFT_KEY, ft_strlen(LEFT_KEY)) && env->line->previous)
		return (true);
...
{
	if (!ft_strncmp(buf, DELETE, ft_strlen(DELETE)) && env->line->next)
		return (true);
